...
// Optimized level metadata lookup using switch
inline const LevelMetadata& get_level_metadata(LogLevel level) noexcept {
    switch (level) {
        ...
        default: {
            static const LevelMetadata metadata{"UNKNOWN", "\x1b[37m", 2};
            return metadata;
        }
    }
}
...
// RAII Performance Profiler - FIXED move semantics
template<LogLevel CompileTimeMinLevel>
class ProfileTimer {
public:
    ...
    // Support chaining like normal log entries - FIXED: return by reference
...
class LogEntryBuilder {
public:
    // FIXED: Remove ambiguity by using only string constructor
    ...
    // Exception integration - FIXED pointer handling
...
public:
    ...
    explicit Logger(std::string name) 
        ...
        , pattern_("{timestamp} [{level}] [{logger}] {message}")  // Default pattern
    {
    ...
    // Create performance profiler (RAII) - FIXED return type
...
private:
    ...
    LogEntryBuilder<CompileTimeMinLevel> create_log_entry(LogLevel level, std::string_view fmt, Args&&... args) {
        ...
        if (static_cast<int>(level) < static_cast<int>(runtime_min_level_.load(std::memory_order_relaxed))) {
            // FIXED: Use explicit string constructor to avoid ambiguity
...
using DefaultLogger = Logger<LogLevel::INFO>;
using DebugLogger = Logger<LogLevel::DEBUG>;
using TraceLogger = Logger<LogLevel::TRACE>;
